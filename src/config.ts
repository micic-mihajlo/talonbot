...
const schemaBase = z.object({
  ...
  FAILED_WORKTREE_RETENTION_HOURS: z.coerce.number().int().min(0).max(24 * 365).default(24),
  TASK_RECOVER_ON_STARTUP: bool.default(true),
  TASK_STUCK_MINUTES: z.coerce.number().int().min(1).max(24 * 60 * 30).default(30),
  TASK_QUEUE_STALE_MINUTES: z.coerce.number().int().min(1).max(24 * 60 * 30).default(30),
  ...
  TRANSPORT_OUTBOX_RETRY_BASE_MS: z.coerce.number().int().min(100).max(120000).default(1000),
  TRANSPORT_OUTBOX_RETRY_MAX_MS: z.coerce.number().int().min(500).max(600000).default(30000),
  TRANSPORT_OUTBOX_MAX_RETRIES: z.coerce.number().int().min(0).max(100).default(10),
  TRANSPORT_OUTBOX_STATE_FILE: z.string().default('~/.local/share/talonbot/transports/outbox.json'),
